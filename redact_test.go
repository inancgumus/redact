...
func TestString_Redacts(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if !strings.ContainsRune(got, redact.DefaultMask) {
				t.Errorf("no mask in output\n input: %q\n   got: %q", c.input, got)
			}
		})
	}
}
...
func TestString_CustomPlaceholder(t *testing.T) {
	...
	out := redact.String(in, redact.Options{Mask: '#'})
	if strings.Contains(out, in) {
		t.Errorf("secret survived: %q", out)
	}
	if !strings.ContainsRune(out, '#') {
		t.Errorf("custom mask missing: %q", out)
	}
	if strings.ContainsRune(out, redact.DefaultMask) {
		t.Errorf("default mask leaked when custom set: %q", out)
	}
}
...
func TestString_RaisedEntropyDisablesGenericMatch(t *testing.T) {
	...
	if strings.ContainsRune(strict, redact.DefaultMask) {
...
func TestString_MultipleInOneInput(t *testing.T) {
	...
	if strings.Count(got, string(redact.DefaultMask)) < 2 {
		t.Errorf("expected at least two mask runes: %q", got)
	}
}
